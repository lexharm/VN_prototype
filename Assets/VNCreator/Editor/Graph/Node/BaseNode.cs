...
namespace VNCreator
{
    ...
    public class NodeViewer : VisualElement
    {
        ...
        public NodeViewer(BaseNode _node)
        {
            ...
            Toggle miniGameField = this.Query<Toggle>("MiniGameToggle").First();
            miniGameField.value = node.nodeData.miniGame;
            Debug.Log("BaseNode: " + node.nodeData.miniGame);
            miniGameField.RegisterValueChangedCallback(
                e =>
                {
                    node.nodeData.miniGame = miniGameField.value;
                }
            );
